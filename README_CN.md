...
本项目是在课题组伪重叠成像与三维数字图像相关研究背景下完成的本科毕业设计软件实现，对应毕业论文《基于深度学习伪重叠成像分离的形貌测量方法》，使用一台实体相机，实现从单张伪重叠散斑图像到三维点云的完整处理流程。

![MonoSpeckle3D 单相机成像原理](assets/monocular_principle.svg)

本项目采用一台实体相机，通过两条反射光路形成两个虚拟视角，再由分色棱镜将清晰和轻微模糊的虚拟视图叠加到同一帧图像中。DivideNet V3 完成分离后，再进行特征匹配和标定三角化。研究方向和光学实验来自课题组的指导与协作，其中导师负责研究方向指导，学长对实验和数据采集提供了重要帮助；本仓库主要呈现毕业设计期间的软件实现、流程整理和开源工程化工作。

## 测量原理：一台相机获得两个虚拟视角

MonoSpeckle3D 并不是普通的单目深度估计网络，它的核心首先是光学测量方案：

- 整套系统只需要 **一台实体相机和一个图像传感器**
- 反射镜和分色棱镜为同一散斑表面构造 **两个虚拟观察视角**
- 两个视角在成像阶段被编码到 **同一张伪重叠图像** 中
- DivideNet V3 将其恢复为清晰分量和轻微模糊分量
- 恢复后的图像对继续提供 SIFT 匹配和标定三角化所需的双视角几何信息

这种设计在保留双视角三维重建信息的同时，减少了实体相机数量、相机同步需求、硬件占用空间以及潜在系统成本。最终精度仍取决于光路安装、图像分离质量和标定质量。

### 光学可分离线索

采集系统在两个虚拟视图之间引入受控的外观差异，避免将两路信息记录为难以区分的完全相同图层。DivideNet V3 使用相互独立的编码器学习清晰与轻度模糊分量的特征分布，同时保留后续 SIFT 匹配所需的散斑结构。

本仓库只说明软件接口和高层成像原理。具体光路对准、光程调节、器件位置和数据采集步骤不属于当前开源范围。这些参数依赖实体实验装置，应通过课题组授权的实验规程确定，而不应根据示例代码直接推断或复现实验。

仓库只保留论文最终采用的技术路线。Deblur、StrainNet、SGBM、ICGN、TV-L1 和 LK 光流等探索后未采用的方法不属于当前代码主线。

原始毕设代码、探索路线和历史脚本完整保存在 [`backup/main-before-paper-refactor-20260918`](https://github.com/lazyerZZZ/MonoSpeckle3D/tree/backup/main-before-paper-refactor-20260918) 分支。如需追溯早期实现或与重构后的主线对照，请查看该分支。
...
本项目是在课题组持续开展的伪重叠成像和三维数字图像相关研究中完成的本科毕业设计。整体研究方向与光学系统构想由课题组提出并指导，实体光路实验和数据采集在学长的重要协助下完成。本仓库所呈现的本科毕设工作主要包括图像分离与三维重建的软件实现、实验分析、论文实现，以及后续面向公开使用的代码重构。

因此，本仓库不表示仓库作者独立提出了完整光学构想。使用本代码或介绍该方法时，请同时了解并引用下列课题组相关论文。
...
第一篇论文建立了课题组的伪重叠多视场 3D-DIC 研究基础，PONet 随后研究了面向同步多视场 3D-DIC 的重叠图像分离问题。MonoSpeckle3D 是采用清晰/轻微离焦分离设置与 SIFT 重建路线的本科毕业设计实现，应当放在这一共同研究背景下理解，而不应被表述为脱离课题组工作的独立优先权主张。

## 致谢

感谢导师对研究方向的提出与指导，感谢学长在光学系统搭建、实验实施和数据采集方面提供的重要帮助。代码和数据集经课题组同意公开。相关协助不代表导师或学长对仓库中每一项工程实现和文字表述负责；如有实现错误，由仓库作者负责。
